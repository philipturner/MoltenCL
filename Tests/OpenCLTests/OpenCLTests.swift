...
final class OpenCLTests: XCTestCase {
    func testExample() throws {
        ...
        clGetPlatformInfo(platform, UInt32(CL_PLATFORM_EXTENSIONS), 0, nil, &extensionsSize)
        ...
        clGetPlatformInfo(
            platform, UInt32(CL_PLATFORM_EXTENSIONS_WITH_VERSION), 0, nil, &extensionsWithVersionSize)
        ...
        withUnsafeTemporaryAllocation(
            of: cl_name_version.self, capacity: extensionsWithVersionSize / clNameVersionStride
        ) { bufferPointer in
            clGetPlatformInfo(
                platform, UInt32(CL_PLATFORM_EXTENSIONS_WITH_VERSION), extensionsWithVersionSize,
                bufferPointer.baseAddress, nil)
            
            XCTAssertEqual(bufferPointer[0].version, CLVersion(major: 1, minor: 0).version)
